import lomakeData from '../../fixtures/yleisavustushakemus.json'

lomakeData.forEach(data => {
    const testName = `Testing: ${data.name}`
    describe(testName, () => {
        beforeEach(function () {
            cy.login('admin')
        });
        ...
        it('Fill form', () => {
            ...
            Object.keys(data.pages).forEach(pageName => {
                const pageData = data.pages[pageName].values
                pageData.forEach( dataItem => {
                    const selector = `[${dataItem.selector.name}="${dataItem.selector.value}"]`

                    if (dataItem.type === 'checkbox') {
                        cy.get(selector).check()
                    }
                    if (dataItem.type === 'textfield') {
                        cy.get(selector).type(dataItem.value)
                    }
                })
